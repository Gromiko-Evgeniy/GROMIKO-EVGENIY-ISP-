...
namespace Lab3_ISP
{
    class Audi : Car, IComparable, IVehicleDocumentation
    {
        public Audi(string _ownerName, string _fyelType, float _fyelQuantity, float _fyelConsumption, float _fuelTankVolume, float _maxSpeed, float _accelerationTimeToMaxSpeed,
       ...
       : base(_ownerName, _fyelType, _fyelQuantity, _fyelConsumption, _fuelTankVolume, _maxSpeed, _accelerationTimeToMaxSpeed)
        ...
        {
            if (obj is Audi AudiObj)
            {
                double mySpeed = MaxSpeed;
                double itsSpeed = AudiObj.MaxSpeed;
                if (mySpeed == itsSpeed)
                {
                    return 0;
                }
